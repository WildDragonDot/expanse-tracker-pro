...
type ExpenseWhereInput = {
  ...
  bank?: string | StringFilter
  paymentMode?: string
  amount?: NumberRangeFilter
  receiptUrl?: null | StringFilter
  tags?: TagsFilter
  OR?: Array<{
    title?: StringFilter
    notes?: StringFilter
...
export function applySorting(transactions: any[], sort: SortOptions): any[] {
  return [...transactions].sort((a, b) => {
    ...
    switch (sort.field) {
      ...
      case 'date':
      default:
        aValue = parseAppDate(a.date).getTime()
        bValue = parseAppDate(b.date).getTime()
        break
    }
  ...
  try {
    ...
    if (saved) {
      ...
      if (parsed.filters.startDate) {
        parsed.filters.startDate = parseAppDate(parsed.filters.startDate)
      }
      if (parsed.filters.endDate) {
        parsed.filters.endDate = parseAppDate(parsed.filters.endDate)
      }
      return parsed
    }
  } catch (error) {
...
export function buildAdvancedPrismaWhere(userId: string, filters: AdvancedExpenseFilters): ExpenseWhereInput {
