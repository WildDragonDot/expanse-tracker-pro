  ...
  const loadNotifications = async () => {
    try {
      ...
      if (res.success && res.notifications && res.notifications.length > 0) {
        ...
        // Compute live notifications directly from local data & occurrences
        const generated: NotificationItem[] = []
        
        try {
          const billsRes = await api.getBillOccurrences()
          if (Array.isArray(billsRes)) {
            const now = new Date()
            billsRes.forEach((bill: any) => {
              const due = new Date(bill.dueDate)
              const diffDays = Math.ceil((due.getTime() - now.getTime()) / (1000 * 60 * 60 * 24))
              if (bill.status === 'OVERDUE' || (diffDays < 0 && bill.status !== 'PAID')) {
                generated.push({
                  id: `bill-overdue-${bill.id}`,
                  type: 'bill',
                  title: `⚠️ Overdue Bill: ${bill.title}`,
                  message: `Payment of ₹${bill.amount.toLocaleString()} was due on ${new Date(bill.dueDate).toLocaleDateString()}. Please clear it promptly.`,
                  createdAt: bill.dueDate,
                  read: false,
                  priority: 'high',
                  actionScreen: 'RecurringBills',
                })
              } else if (bill.status === 'UPCOMING' && diffDays >= 0 && diffDays <= 7) {
                generated.push({
                  id: `bill-upcoming-${bill.id}`,
                  type: 'bill',
                  title: `🔔 Upcoming Bill: ${bill.title}`,
                  message: `Payment of ₹${bill.amount.toLocaleString()} is due ${diffDays === 0 ? 'today' : `in ${diffDays} day${diffDays > 1 ? 's' : ''}`}.`,
                  createdAt: bill.dueDate,
                  read: false,
        ...
        try {
          const now = new Date()
          const budgets = await api.getMonthlyBudgets(now.getMonth() + 1, now.getFullYear())
          if (Array.isArray(budgets)) {
            budgets.forEach((b: any) => {
              const pct = b.amount > 0 ? (b.spent / b.amount) * 100 : 0
              if (pct >= 100) {
                generated.push({
                  id: `budget-over-${b.id || b.category}`,
                  type: 'budget',
                  title: `🚨 Budget Exceeded: ${b.category}`,
                  message: `You have spent ₹${b.spent.toLocaleString()} (${Math.round(pct)}%) of your ₹${b.amount.toLocaleString()} budget limit.`,
                  createdAt: new Date().toISOString(),
                  read: false,
                  priority: 'high',
                  actionScreen: 'MonthlyBudget',
                })
              } else if (pct >= 80) {
                generated.push({
                  id: `budget-warn-${b.id || b.category}`,
                  type: 'budget',
        ...
        if (generated.length === 0) {
          generated.push({
            ...
            message: 'All your bills, budgets, and expenses are actively tracked with real-time alerts.',
            createdAt: new Date().toISOString(),
            read: true,
            priority: 'low',
          })
